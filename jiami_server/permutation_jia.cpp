...
#include "ui_permutation_jia.h"
#include <QFile>
#include <QFileDialog>
Permutation_jia::Permutation_jia(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Permutation_jia)
{
    int e = 1;
    change(e);
    ui->setupUi(this);
    ...
    connect(tcpServer,&QTcpServer::newConnection,[=](){
        ...
        connect(tcpSocket,&QTcpSocket::readyRead,[=](){
            ...
            QString cmd = "./permutation 1 "+array;
           // qDebug()<<cmd;
            process->start(cmd);
            out = tr("");
...
void Permutation_jia::on_decrption_clicked()
{
    if(NULL == tcpSocket){
        return;
    }
    ...
    QString cmd = "./permutation 2 "+ui->keyword->text()+" "+str;
    //qDebug()<<cmd;
    process->start(cmd);
    out=tr("");
}

void Permutation_jia::on_cut_clicked()
{
    if(tcpSocket == NULL){
        return;
    }
...
void Permutation_jia::readOutput(){
   out += process->readAll();
   if(flag == 1){
   ui->ciphertext->setText(out);
   }
...
void Permutation_jia::change(int i){
    flag = i ;
}

void Permutation_jia::on_inFile_clicked()
{
...
void Permutation_jia::on_outFile_clicked()
{
...
void Permutation_jia::on_zero_clicked()
{
