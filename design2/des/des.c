...
int main(int argc,char * argv[])
{
 ...
 if(argc<4){
     printf("1:encrption/2:decrption keyword(8 byte) plaintext/ciphertext\n");
     return 0;
}  

 if(atoi(argv[1])==1){
// printf("请输入你要加密的内容(8 Byte):\n");
// fgets(My_message,9,stdin);
   strcpy(My_message,argv[3]);
// int a;
// a=strlen(My_message);
// printf("%d\n",a);

// printf("请输入你的加密密钥:\n");
 //fgets(My_key,9,stdin);
 //i=strlen(My_key);
 //printf("%d",i);
  strcpy(My_key,argv[2]);
  i = strlen(My_key);
 while(i!=8)
 {
 ...
 //fgets(You_key,9,stdin);
strcpy(You_key,argv[2]);
 i=strlen(You_key);
 while(i!=8)
 {
 ...
 Run_desDes(My_message,argv[3]);//解密;
 printf("解密结果为:\n");
 for(i=0;i<8;i++)
 {
